...
class SpeechService {
  ...
  bool _shouldAutoCalculate(String expression) {
    ...
    RegExp completeExpression = RegExp(r'^\d+(\.\d+)?\s*[+\-×÷%]\s*\d+(\.\d+)?$');
    if (completeExpression.hasMatch(clean)) {
      return true;
    }

    // Check for more complex expressions
    RegExp multiOperation = RegExp(r'^\d+(\.\d+)?(\s*[+\-×÷%]\s*\d+(\.\d+)?)+$');
  ...
  String _replaceOperatorWords(String text) {
    ...
    for (var entry in sortedEntries) {
      ...
      result = result.replaceAll(RegExp(pattern, caseSensitive: false), entry.value);
    }
  ...
  String _handleMathematicalPhrases(String text) {
    ...
    phraseMap.forEach((phrase, replacement) {
      result = result.replaceAll(RegExp('\\b${RegExp.escape(phrase)}\\b', caseSensitive: false), replacement);
    });
  ...
  String _cleanExpression(String text) {
    // Remove common filler words
    final fillerWords = ['the', 'a', 'an', 'to', 'from', 'with', 'please', 'can you'];
    String result = text;

    for (String word in fillerWords) {
      // Use precise word boundary matching
      result = result.replaceAll(RegExp('\\b$word\\b', caseSensitive: false), '');
    }
